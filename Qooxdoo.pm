...
sub onAuthenticated {
      ...
      )
    {
        $poe_kernel->yield( sendToQX => "addbutton "
              . CGI::escape("") . " "
              . CGI::escape("richibutton") . " "
              . CGI::escape("Projekte") . " "
              . CGI::escape("") . " "
              . CGI::escape("job=show,table=projects") );
    }
...
sub onClientData {
    ...
    {
        Log(
            "onClientData: Missing parameters: connection heap="
              . $options->{heap}
              . ":session="
              . $options->{curSession} . ": !",
            $ERROR
        ...
        $self->{dbm}->setFilter(
            {
                ...
                table      => "transactions",
                filter =>
                  { "transactions" . $TSEP . "project_id" => $options->{id} }
            }
        );
...
# get the name of the clicked table (we know only the id) from the Database Backend
        ...
        $self->onShow(
            {
                ...
                curSession  => $options->{curSession},
                connection  => $options->{connection},
                windowtitle => "Beteiligungen an: $project_name"
            }
        );
        return;
    }
    ...
    if ( ( $options->{job} eq "saveedit" ) || ( $options->{job} eq "newedit" ) )
    {
        if ( $options->{table} eq "transactions" ) {
            ...
            $params->{columns}->{ $options->{table}
                  . $TSEP
                  . $self->{dbm}->getIdColumnName( $options->{table} ) } = $id;
...
sub onDelRow {
    ...
    unless ( ( !$moreparams ) && $options->{table} && $options->{curSession} ) {
        Log(
            "onDelRow: Missing parameters: connection:"
              . $options->{table} . ": !",
            $ERROR
        );
        return undef;
...
sub onShow {
    ...
    {
        Log(
            "onShow: Missing parameters: table:"
              . $options->{table}
              . ":curSession:"
              . $options->{curSession}
              . ":connection:"
    ...
    if ( $options->{table} eq "transactions" ) {
        ...
        my $footer_identifier =
          $window_identifier . "_data_" . "amount_missing";

        # create an new iframe object on the client
        $poe_kernel->yield( sendToQX => "createiframe "
              . $footer_identifier . " "
              . CGI::escape("about:blank") );
...
"<table width=100%><tr><td>Fehlender Betrag: </td><td align=right> <b><font color=red>$amount_missing &euro; </font></b> </td></tr></table>";
    ...
    $poe_kernel->yield( sendToQX => "iframewritereset "
          . $footer_identifier . " "
...
sub getTableButtonsDef {
    ...
    if ( $options->{table} eq "projects" ) {
        if ( $self->{dbm}->{config}->{nojson} ) {
            ...
            return [
                CGI::escape( $self->{text}->{qx}->{new_project} ) . ","
                  . CGI::escape("Bearbeiten") . ","
                  . CGI::escape("Löschen") . ","
                  . CGI::escape("Filter"),
                CGI::escape("resource/qx/icon/Tango/32/actions/list-add.png")
                  ...
                  . CGI::escape(
                    "resource/qx/icon/Tango/32/actions/list-remove.png")
                  . ","
                  . CGI::escape(
        ...
        push(
            ...
            (
                ...
                },
                {
                    ...
                    label => "Bearbeiten",
                    image =>
                      ( $options->{smallbuttons} ? "" : "/bilder/edit.png" ),
                    action => "neweditentry",
                    bindto => "row",
                },
                {
        ...
        if ( $self->{dbm}->{config}->{nojson} ) {
            ...
            return [
                CGI::escape("Beteiligen") . ","
                  . CGI::escape("Beteiligung bearbeiten") . ","
                  . CGI::escape("Löschen") . ","
                  . CGI::escape("Filter"),
                CGI::escape("resource/qx/icon/Tango/32/actions/list-add.png")
                  . ","
